...
def build_graph(layout_str, slanted):
    ...
    * on keypad layout, '7' maps to [None, None, None, '=', '8', '5', '4', None]
    ...
    # x position unit len is token len plus 1 for the following whitespace.
    x_unit = token_size + 1
    adjacency_func = get_slanted_adjacent_coords if slanted else get_aligned_adjacent_coords
    assert all(len(
        token) == token_size for token in tokens), 'token len mismatch:\n ' + layout_str
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'w') as f:
        ...
        for graph_name, args in [('qwerty', (qwerty, True)),
            ...
            lines.append('export const %s: Record<string, (string | null)[]> = %s' %
